...
"""  # noqa
    template_cmd = """\
DEVITO_MPI=%(mpi)s mpiexec amplxe-cl -collect hpc-performance -r %(resultdir)s -- python benchmark.py bench -P %(problem)s -bm O2 -d %(shape)s -so %(space_order)s --tn %(tn)s -x 1 --arch %(arch)s -r %(resultsdir)s\
"""  # noqa

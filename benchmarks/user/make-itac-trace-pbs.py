...
module load mpi
source /apps/intel/2017.6/parallel_studio_xe_2017.6.064/bin/psxevars.sh
LD_PRELOAD=/apps/intel/2017.6/parallel_studio_xe_2017.6.064/itac_2017/slib/libVT.so
...
cd benchmarks/user
export VT_LOGFILE_NAME=trace%(nn)s_%(np)s_%(mpi)s.stf

"""  # noqa
    template_cmd = """\
DEVITO_MPI=%(mpi)s mpiexec.hydra -trace "libVT.so libmpi.so" -r %(resultdir)s python benchmark.py bench -P %(problem)s -bm O2 -d %(shape)s -so %(space_order)s --tn %(tn)s -x 1 --arch %(arch)s -r %(resultsdir)s\
"""  # noqa

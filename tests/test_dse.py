...
from devito.passes.iet import IncrDimension
...
def test_cse(tu, tv, tw, ti0, ti1, t0, t1, exprs, expected):
    """Test common subexpressions elimination."""
    counter = generator()
    make = lambda: Scalar(name='r%d' % counter()).indexify()
    processed = _cse(EVAL(exprs, tu, tv, tw, ti0, ti1, t0, t1), make)
...
def test_tti_rewrite_aggressive(tti_nodse):
    ...
    block_dims = [i for i in op.dimensions if isinstance(i, IncrDimension)]

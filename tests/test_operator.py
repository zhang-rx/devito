...
class TestDeclarator(object):
    ...
    def test_heap_perfect_2D(self):
        ...
        assert op.body[1].is_List
        assert str(op.body[1].header[0]) == 'float (*a);'
        assert str(op.body[1].header[1]) == 'float (*c)[j_size];'
        assert str(op.body[1].header[2]) == ('posix_memalign((void**)&a, 64, '
                                             'sizeof(float[i_size]));')
...
class TestLoopScheduling(object):
    ...
    @pytest.mark.parametrize('shape, dimensions', [((11, 11), (x, y)),
                                                   ...
                                                   ((11, 11, 11), (z, x, y)),
                                                   ((11, 11, 11), (z, y, x))])
    def test_equations_mixed_functions(self, shape, dimensions):
        """
